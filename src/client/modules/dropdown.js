    ...
    if (event.target.className.indexOf("no-close") < 0) {
      if (!event.target.matches(".dropbtn")) {
        ...
        if (
          parent &&
          parent.className &&
          parent.className.indexOf("dropbtn") >= 0
        ) {
          showDropdown(parent, dropdowns)
        } else if (event.target.matches(".dropbtn-close")) {
          closeDropdown()
        } else {
          if (
...
function showDropdown(target, dropdowns) {
  ...
  var coordinateMain = main.getBoundingClientRect()
  removeAllActive()
  ...
  for (var i = 0; i < current.childNodes.length; i++) {
    if (current.childNodes[i].className.indexOf("dropdown-content") >= 0) {
      if (child[i] === "none" || child[i] === "") {
        ...
        var coordinateCurrent= current.childNodes[i].getBoundingClientRect()
        if (coordinateCurrent.right > coordinateMain.right) {
          current.childNodes[i].style.left = "unset"
        }
      } else {
